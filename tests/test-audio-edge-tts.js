/**
 * 🧪 Test Script for Voice Chatbot with Edge-TTS (Arabic Support)
 * 
 * This script:
 * 1. Gets AI response from Vertex AI Gemini API (using service account)
 * 2. Converts response text to audio using Edge-TTS (excellent Arabic support)
...
const { textToSpeech, getAvailableVoices, DEFAULT_VOICE } = require('./backend/tts-edge-handler');
...
// ============================================
// 🔊 Step 2: Convert Text to Audio (Edge-TTS)
// ============================================
...
async function convertTextToAudio(text) {
    ...
    try {
        // Use Edge-TTS for excellent Arabic pronunciation
        const result = await textToSpeech(text, DEFAULT_VOICE, OUTPUT_AUDIO_FILE);
        
        console.log(`✅ [STEP 2] Audio converted successfully!`);
...
function listVoices() {
    console.log('\n' + '='.repeat(70));
    console.log('🎤 Available Arabic Voices (Edge-TTS)');
    console.log('='.repeat(70) + '\n');

    const voices = getAvailableVoices();
    
    voices.forEach((voice, index) => {
        console.log(`${index + 1}. ${voice.country} - ${voice.gender}`);
...
async function main() {
    console.log('='.repeat(70));
    console.log('🧪 Voice Chatbot Audio Pipeline Test (Edge-TTS Version)');
    console.log('='.repeat(70));
    console.log('');
    ...
    try {
        ...
        // Check if --list-voices flag is provided
        ...
        // Step 2: Convert response to audio using Edge-TTS
        await convertTextToAudio(response);
        console.log('');
        ...
        console.log('💡 Edge-TTS provides excellent Arabic pronunciation!');
        console.log('='.repeat(70));
